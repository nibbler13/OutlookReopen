...
Local $sOutlookProcess = "outlook.exe"
Local $sUserName = @UserName

While True
	Sleep(60 * 1000)

	Local $bProcessFounded = False

	Local $aProcessList = _ProcessListProperties($sOutlookProcess)
;~ 	_ArrayDisplay($aProcessList)
	If IsArray($aProcessList) And UBound($aProcessList, $UBOUND_COLUMNS) > 3 Then
		For $i = 1 To UBound($aProcessList, $UBOUND_ROWS) - 1
			If StringInStr($aProcessList[$i][3], $sUserName) Then
				$bProcessFounded = True
				ExitLoop
			EndIf
		Next
	EndIf

	If Not $bProcessFounded Then ShellExecute($sOutlookProcess)
;~ 	$aProcessList = 0
...
Func _ProcessListProperties($Process = "", $sComputer = ".")
    ...
    Local $oWMI = ObjGet("winmgmts:{impersonationLevel=impersonate,authenticationLevel=pktPrivacy, (Debug)}!\\" & $sComputer & "\root\cimv2")
    If IsObj($oWMI) Then
